...
class InputsPage(QWidget):
    def __init__(self):
        ...
        # Static dropdown expanded preview
        layout.addWidget(self._section("下拉列表展开预览"))
        preview_note = QLabel("（下拉展开后会自动关闭，无法截图。以下为静态模拟展示）")
        preview_note.setStyleSheet(f"color: {C['overlay0']}; font-size: 12px;")
        layout.addWidget(preview_note)

        dp_row = QHBoxLayout()
        dp_row.setSpacing(16)

        # Simulate a closed combo
        closed_combo = QFrame()
        closed_combo.setStyleSheet(f"""
            QFrame {{
        ...
        """)
        closed_combo.setFixedWidth(220)
        closed_combo.setFixedHeight(36)
        cc_layout = QHBoxLayout(closed_combo)
        cc_layout.setContentsMargins(12, 0, 8, 0)
        cc_text = QLabel("Catppuccin Mocha")
        cc_text.setStyleSheet(f"color: {C['text']}; font-size: 13px; border: none;")
        cc_layout.addWidget(cc_text)
        cc_layout.addStretch()
        cc_arrow = QLabel("▾")
        cc_arrow.setStyleSheet(f"color: {C['overlay0']}; font-size: 12px; border: none;")
        cc_layout.addWidget(cc_arrow)

        # Simulate expanded dropdown
        dropdown_col = QVBoxLayout()
        dropdown_col.setSpacing(4)
        dropdown_col.addWidget(closed_combo)

        dropdown_popup = QFrame()
        dropdown_popup.setStyleSheet(f"""
            QFrame {{
                background: {C['surface0']};
        ...
        """)
        dropdown_popup.setFixedWidth(220)
        dp_inner = QVBoxLayout(dropdown_popup)
        dp_inner.setContentsMargins(4, 4, 4, 4)
        dp_inner.setSpacing(2)
        dropdown_items = ["Catppuccin Mocha", "Catppuccin Latte", "Nord", "Dracula", "One Dark"]
        for i, item_text in enumerate(dropdown_items):
            item_lbl = QLabel(item_text)
            if i == 0:
                item_lbl.setStyleSheet(f"""
                    QLabel {{
                ...
                """)
            dp_inner.addWidget(item_lbl)
        dropdown_col.addWidget(dropdown_popup)
        dropdown_col.addStretch()
        dp_row.addLayout(dropdown_col)
        dp_row.addStretch()
